...
class SegmentationModule(LightningModule):
    ...
    def common_step(self, batch, batch_idx, step_name):
        ...
        # Create the third class: neither of the two classes is active
        neither_class = (y.sum(dim=-1, keepdim=True) == 0).float()

        # Concatenate the neither class to the y tensor

        y = torch.cat([neither_class, y], dim=-1)
