...
def main(cfg):
    ...
    insitu_cfg = cfg["base_dataset"]["dataset"]["single_signal_datasets"][1]
    ...
    catalog_cfg = cfg["base_dataset"]["dataset"]["single_signal_datasets"][0]

    catalog_dataset = ICMECAT_Dataset(
        folder_paths=catalog_cfg["folder_paths"],
        event_types=catalog_cfg["event_types"],
    )

    multi_signal_dataset = MultiSignalDataset(
        single_signal_datasets=[catalog_dataset, insitu_data],
        catalog_idx=0,
        aggregation="I:1",
        fill="zero",
    )
    ...
    multi_signal_dataset_high_res = MultiSignalDataset(
        single_signal_datasets=[catalog_dataset, high_res_insitu_data],
        catalog_idx=0,
        aggregation="I:1",
        fill="zero",
    )
    ...
    for fold in range(3):
        ...
        with torch.no_grad():
            ...
            all_results_df = pd.DataFrame(index=pd.to_datetime(timestamps, unit="s"))
                ...
                for i, batchitem in enumerate(idxs):
                    ...
                    try:
                        small_df = pd.DataFrame(
                            ...
                            columns=[f"predicted_value_{batchitem_int}"],
                        )
                    except:
                        breakpoint()

                    all_results_df = pd.concat([all_results_df, small_df], axis=1)

            all_results_df.index = all_results_df.index.round("30min")

            aggregated_results = all_results_df.mean(axis=1)
        ...
        aggregated_results = aggregated_results.reindex(expected_index)
        df = df.reindex(expected_index, method="ffill")
        aggregated_results = aggregated_results.ffill()

        # Convert aggregated_results (Series) to a DataFrame
        aggregated_results_df = aggregated_results.to_frame(
            name=f"predicted_value_{fold}"
        )

        merged_df = pd.merge(
            aggregated_results_df, df, how="outer", left_index=True, right_index=True
        )
    ...
    cols_to_merge = [f"predicted_value_{fold}" for fold in range(3)]
    merged_df["predicted_value"] = merged_df[cols_to_merge].mean(axis=1)
    ...
    for event in catalog:
        event.proba = merged_df.loc[event.begin : event.end, "predicted_value"].mean()
        event.proba_max = merged_df.loc[
            event.begin : event.end, "predicted_value"
        ].max()
    ...
    for event in catalog:
        ...
        event.event_id = f"ICME_RTSW_ARCANE_{event_date_str}_{event_number:02d}"
    ...
    fig = make_subplots(rows=len(variables) + 2, cols=1, shared_xaxes=True)
    ...
    fig.add_trace(
        go.Heatmap(
            z=merged_df["predicted_value"].values.reshape(1, -1),
            x=merged_df.index,
            y=["Predicted Value"],
            zmin=0,
            zmax=1,
    ...
    fig.update_yaxes(
        showticklabels=False, title_text="ARCANE<br>Probability", row=i + 2, col=1
    )

    fig.update_yaxes(
        showticklabels=False, title_text="Threshold<br>Probability", row=i + 3, col=1
    )
    ...
    fig.write_html(cache_path / Path("arcane_plot_now.html"))
    fig.write_image(cache_path / Path("arcane_plot_now.png"))

    update_event_catalog(cache_path / Path("arcane_catalog_now.csv"), catalog)
